...
const BLACK_LIST_RUBY_CIRC_SUPPLY = {
    list: [
...
async function createTokenHoldersDB(FILENAME, explorerURL, tokenAddress) {
    let limit = 500;
    ...
    while (limit === 500) {
        ...
        const holders = await getTokenHolders(explorerURL, tokenAddress, index, limit).then(res => {
            return res;
        }).catch(err => {
    ...
    console.log("Create TokenHoldersDB api requests required:", index)
    ...
    console.log("Holders: write to file:", res,currentDate )
...
// api calls
async function getTokenHolders(explorerURL, address, page, offset) {

    const stringThis = '?module=token&action=getTokenHolders&contractaddress=' + address + "&page=" + page + "&offset=" + offset

    const res = await axios.get(explorerURL + stringThis, {
...
// match addresses 
async function blicklistRubyAddresses(walletAddress) {

    const length = BLACK_LIST_RUBY_CIRC_SUPPLY.list.length;
    let bl;
    for (let i = 0; i < length; i++) {

        bl = BLACK_LIST_RUBY_CIRC_SUPPLY.list[i].toLowerCase();
...
async function sortTokenHolders(token_address, FILENAME, FILENAME_HOLDERS, init_signer) {

    const res = await utils.readJsonFile(FILENAME_HOLDERS);
    ...
    if (typeof length === 'number') {
        for (let i = 0; i < length; i++) {
            ...
            for (let ii = 0; ii < length; ii++) {
                ...
                let isBlocked = await blicklistRubyAddresses(address);
                if (!isBlocked) {
        ...
        const burnt = Number(await amm.checkTokenBurnedAmount(token_address, init_signer));

        const obj = {
            circulating_supply: ruby_circulatiing,
            total_supply: total_supply,
            burned: burnt,
            holders: address_total
        }

        const res1 = await utils.writeJsonFile(FILENAME, obj).then(res => {
            return res;
        }).catch(err => {
            console.log("Error : writeJsonFile ", err);
        })

        console.log("Circulating Ruby: write to file:", res1)
    } else {
        console.error("Circulating Ruby: failed write to file:", res1)
    }
}

module.exports.createTokenHoldersDB = createTokenHoldersDB;
module.exports.sortTokenHolders = sortTokenHolders;